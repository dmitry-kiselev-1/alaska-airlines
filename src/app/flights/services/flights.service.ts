...
export class FlightsService extends BaseHttpService {
  ...
  public getHealthPing(): Observable<string> {
    return this.http
      .get<string>(environment.flightsApiHealthCheckEndpoint)
      .pipe(
  ...
  public async getFlights(
    model: FlightsSearchModel
  ): Promise<FlightsSearchResultModel[]> {
    ...
    const getFlightsModel = await firstValueFrom<GetFlightsModel>(
      this.http
        .get<any>(
          environment.useFlightsApiEndpointJson
            ? './assets/flights.json'
            : query
        )
        .pipe(
    ...
    const result: FlightsSearchResultModel[] = getFlightsModel.flights.reduce(
      (accumulator: any[], curent: any) => [
        ...accumulator,
        {
          FlightNumber: curent.flightDetails.operatingFlightNumber,
          DepartureTime:
            curent.flightDetails.flightLegs[0].scheduledDateTimeUTC.in,
          ArrivalTime:
            curent.flightDetails.flightLegs[0].scheduledDateTimeUTC.out,
        },
      ],
      []
    );
